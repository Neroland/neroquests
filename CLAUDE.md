# Project context for AI coding agents — neroquests
...
- Loader entry points: `NeroQuestsFabric` (+ `NeroQuestsFabricClient`), `NeroQuestsForge`,
  `NeroQuestsNeoForge` — each calls `NeroQuestsCommon.init()` during construction.
- NeoForge/Forge debug tasks use `-PneroquestsDebug`; Fabric Loom honours Gradle `--debug-jvm`.

## IDE (VS Code) run & debug

- Workspace: **`neroquests.code-workspace`** (single-root `"."`). Import the Stonecutter nodes as **static
  ...
  VS Code. Per-node Eclipse project names are `neroquests-<loader>-<mc>`.
...
- This mod has its own **dedicated wiki** in `wiki/` at the repo root: the player- and
  contributor-facing docs for NeroQuests (features, blocks/items, machines, progression, recipes, FAQ).
...
- The wiki is **per-mod** — document only NeroQuests here; cross-mod / ecosystem concepts belong in the
